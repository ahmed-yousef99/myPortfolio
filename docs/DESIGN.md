...
This document establishes the static visual foundations, brand identity, color system, typography, and styling limits of the portfolio canvas. For motion behavior, component catalogs, and interactivity models, refer to:
- [MOTION.md](file:///c:/dev/Portfolio/docs/MOTION.md) — 3D & animation specifications.
- [COMPONENTS.md](file:///c:/dev/Portfolio/docs/COMPONENTS.md) — UI component specifications & tokens.
...
The marketing canvas is designed as a software-craft documentation site: dense, technical, and quietly luxurious. It relies on structure, visual hierarchy, and alignment to tell a story rather than decorative flourishes. 

### Design Goals
Every visitor should describe the interface as:
- **Engineered** — Built with mathematical grid alignment and modular logic.
- **Premium** — Quietly luxurious through subtle highlights and hairline details.
- **Interactive** — Reacts directly to the user's presence and controls.
- **Modern** — Incorporating dark surface paradigms and custom tracking.
- **Alive** — Subtly shifts with perspective, shadows, and reflections.
- **Three-dimensional** — Uses physical layers, volumetric shapes, and consistent lighting directions.
- **Calm** — Reduced chromatic noise, avoiding unnecessary visual competition.
- **Precise** — Structured spacing limits and standardized curves.

**Avoid:**
- Flashy or gimmicky elements.
- Overloaded information structures.
- Flat visual planes lacking depth.

---

## Brand Identity

Linear's marketing canvas uses a dark-canvas system to direct all focus toward product captures. The surrounding interface frames the content, acting as a clean, luxurious bezel.
- **Dark-canvas system**: `{colors.canvas}` (#010102) acts as the deepest dark surface in the system.
- **Accent hierarchy**: The signature lavender-blue color is strictly rationed. It serves as a visual anchor and focus indicator rather than a recurring element.
- **Product focus**: The page rhythm is built around high-fidelity captures of product interfaces framed in clean dark panels. The website chrome is minimal to let screenshots dominate.

---

## Color System

The system operates on a curated, custom color palette that establishes surface depth and typography contrast.
...
| Level | Treatment | Use |
| :--- | :--- | :--- |
| **Level 0** (flat) | No shadow, no border | Default for body type, hero text, footer |
| **Level 1** (charcoal lift) | `{colors.surface-1}` background on canvas, 1px `{colors.hairline}` | Default cards, product panels |
| **Level 2** (surface-2 lift) | `{colors.surface-2}` background, 1px `{colors.hairline-strong}` | Featured pricing card, hovered cards |
| **Level 3** (surface-3 lift) | `{colors.surface-3}` background | Sub-nav, dropdown menus |
| **Level 4** (focus ring) | 2px `{colors.primary-focus}` outline at 50% opacity | Focused input, focused button |

Linear's depth is carried by the surface ladder and hairline borders. The brand resists drop shadows on dark surfaces almost entirely.

### Decorative Depth
- Product UI screenshots dominate as decorative depth.
- No atmospheric gradients, no spotlight cards.
- **Subtle white edge highlight** on the top edge of lifted panels — gives the dark surface a faint "pixel rendered" feel. Refer to [MOTION.md](file:///c:/dev/Portfolio/docs/MOTION.md) for 3D physics and lighting source directions.
...
- Light mode is not documented because the marketing site does not ship a theme.
